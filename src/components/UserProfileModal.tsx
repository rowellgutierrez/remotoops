...
import { db, doc, setDoc, addDoc, collection, storage, ref, uploadBytes, getDownloadURL } from '../lib/firebase';
...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  const handleAvatarFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Fast browser-side image compression & resizing (max 800px)
      const { blob, previewUrl } = await compressAndResizeImage(file, 800, 0.82);
      // Immediate local preview
      setAvatar(previewUrl);

      const cleanName = file.name.replace(/[^a-zA-Z0-9._-]/g, '');
      const storageRef = ref(storage, `users/${currentUser.id}/avatar_${Date.now()}_${cleanName}.jpg`);
  ...
  const handleCoverFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Fast browser-side image compression & resizing (max 1600px)
      const { blob, previewUrl } = await compressAndResizeImage(file, 1600, 0.82);
      // Immediate local preview
      setCoverImage(previewUrl);

      const cleanName = file.name.replace(/[^a-zA-Z0-9._-]/g, '');
      const storageRef = ref(storage, `users/${currentUser.id}/cover_${Date.now()}_${cleanName}.jpg`);
      await uploadBytes(storageRef, blob, { contentType: 'image/jpeg' });
      const url = await getDownloadURL(storageRef);
      setCoverImage(url);
    } catch (err) {
      console.error("Cover upload error:", err);
      alert("Failed to upload cover banner. Please try again.");
    } finally {
        ...
        <form onSubmit={handleSaveProfile} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-slate-50">
            ...
            <div className="relative h-44 w-full bg-slate-200 overflow-hidden group">
              ...
              <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <label className="bg-white/95 hover:bg-white text-slate-900 text-xs font-bold px-3 py-2 rounded-xl shadow cursor-pointer flex items-center gap-1.5 transition-all">
              ...
              <div className="absolute top-3 right-3 flex items-center gap-1 bg-white/90 backdrop-blur-md p-1.5 rounded-xl border border-slate-200 shadow-sm">
                <span className="text-[10px] text-slate-700 font-bold px-1.5 flex items-center gap-1">
                  <ImageIcon className="w-3 h-3 text-teal-600" /> Presets:
                </span>
                {DEFAULT_COVER_IMAGES.map((img, idx) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-lg overflow-hidden border-2 transition-all ${
                      coverImage === img ? 'border-teal-600 scale-110' : 'border-slate-300 opacity-70 hover:opacity-100'
                    }`}
                  >
            ...
            <div className="px-6 pb-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-16 relative z-10">
              <div className="flex items-end gap-4">
                <div className="relative group">
                  ...
                  />
                  <label className="absolute inset-0 bg-slate-900/70 rounded-2xl opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center cursor-pointer text-white text-[10px] font-bold p-1 text-center">
          ...
          <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">Full Name / Display Name</label>
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  className="w-full bg-white border border-slate-300 rounded-xl px-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500"
                  required
                />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">Location / Timezone</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 rounded-xl pl-9 pr-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500"
              ...
              <div className="sm:col-span-2">
                <label className="text-xs font-bold text-slate-700 mb-1 block">Headline / Professional Title *</label>
                <input
                  type="text"
                  value={headline}
                  onChange={(e) => {
