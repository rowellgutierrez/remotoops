...
export const MessagesSection: React.FC<MessagesSectionProps> = ({
  ...
  const currentConversation = conversationsMap.get(activeConversationId) || (conversationKeys.length > 0 ? conversationsMap.get(conversationKeys[0]) || [] : []);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const receiverId = firstMsg 
      ? (firstMsg.senderId === currentUser.id ? firstMsg.receiverId : firstMsg.senderId) 
      : 'employer';
    const receiverName = firstMsg 
      ? (firstMsg.senderId === currentUser.id ? 'Employer' : firstMsg.senderName) 
      : 'Employer';

    onSendMessage(messageInput.trim(), receiverId, receiverName, activeConversationId || 'conv-general');
    setMessageInput('');
  };
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {conversationKeys.length === 0 ? (
              <div className="p-6 text-center text-xs text-slate-400 space-y-2">
                ...
                <p className="text-[11px] text-slate-400">Applications and interview chats appear here.</p>
              </div>
            ) : (
          ...
          {conversationKeys.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4 min-h-[400px]">
              ...
              <div className="space-y-1 max-w-sm">
                <h3 className="font-bold text-slate-800 text-sm">No Messages Selected</h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  When you submit job applications or receive interview invitations, your direct communications with employers will appear here in real-time.
                </p>
              </div>
            </div>
          ) : (
